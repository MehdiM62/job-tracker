...
- CV Language toggle (EN / DE) plus a separate CV Version dropdown (remembers versions you've typed in before, with your last pick preselected), so you can track which specific CV variant performs better
...
| M | CV Version | Which CV file/variant you used — dropdown, with a text box to add a new one |
| N | Source | Auto-detected from URL, or your Source dropdown choice |
| O | Company Comments | Appended history from the Update from Email tab |
| P | Match Level | % fit vs. your career profile (if configured) |
| Q | Missing Skills | Gaps vs. your career profile (if configured) |

Columns M (CV Version) and N (Source) must exist in your sheet's header row before you
start — add them as the headers of columns M and N if they aren't there yet. Columns O–Q
(Company Comments, Match Level, Missing Skills) are created automatically on first run if
...
- **CV Version**: dropdown next to CV Language for which specific CV file/variant you used — keeps this independent of CV Language so you can compare interview rates by CV version, by language, or both. Pick "Enter new version..." to type a name not listed yet; it's remembered (in a hidden `_cv_versions` sheet tab) and offered as a normal option from then on, and your last pick is preselected next time
...
- **Duplicate protection and mobile reliability**: both Add Job and Update from Email check the live sheet for a near-identical existing row (same URL, or same company + role) before letting you submit, showing a warning plus an explicit "I know — add anyway" checkbox rather than silently blocking or silently proceeding. The check itself is retried on a transient connection error rather than failing silently — if every attempt still fails, you get a "could not check for duplicates" warning instead of a false all-clear. Separately, if your browser's session gets dropped mid-flow (common on mobile when the tab is backgrounded — screen lock, app switch), a finished Fetch & Parse or Parse Email result is recovered automatically on your next visit to that tab instead of just vanishing, with a banner noting it was restored — this now also covers a session dying mid-"matching against your profile" or mid-duplicate-check, not just after a fully finished parse. If the session instead drops *during* Fetch & Parse or Parse Email itself (most likely while the slow AI step is running) rather than after it finishes, revisiting that tab automatically re-launches the same attempt from scratch (URL/pasted text/email and CV language included) instead of leaving you stuck watching a spinner that will never finish, or needing to retype everything. On top of all of that, **every** row-creating write (Add Job, Update from Email's "add as new", and the Bulk Email Update tab) does one final check against the live sheet immediately before writing — no matter what caused a retry (a slow save that looked stuck, a recovered session, this account open on two devices at once), creating the same application twice within a few days silently does nothing instead of adding another row; no Sheets request used by the app can hang forever either, so a stalled connection now fails within seconds instead of leaving "Saving to Google Sheet..." stuck indefinitely. Finally, a browser tab backgrounded for a while (screen lock, app switch) during Fetch & Parse, Parse Email, or a submit itself can come back to foreground still showing that same stale in-progress spinner even once the work is actually done — the tab's own rendering was throttled while hidden, and nothing repaints it until some other interaction happens. Coming back to a tab that was hidden for 20+ seconds during one of those now reloads it automatically once, which cleanly re-enters the recovery behavior above instead of leaving you thinking it's still running (never while editing an already-parsed review form, so an in-progress edit there is never discarded by this).
...
- **Missing info an email fills in is shown in the form, not just silently applied**: if the matched row has a currently-blank/unspecified field (most often Contact Person — e.g. a row added without a recruiter name) and the email actually names one, the review form shows an extra editable field for it, pre-filled with what the AI extracted, right alongside the status and comments — so you see and can correct it before clicking **Update Sheet**, rather than only finding out afterward from the "also filled in ..." note. A field that already has real content on the row is never shown here and never touched, no matter what the email says.
