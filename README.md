...
- **Update from Email**: paste the email as-is (subject + body); the AI reads the email's own date, matches it to a job, and appends a new dated entry to Company Comments — it never overwrites earlier entries, so you get a full timeline per application. OpenRouter (the primary provider) gets your *entire* job history as match candidates (capped at 100K characters as a sanity ceiling, well within its 131K-token context) — only the Groq fallback, which has a tight per-minute token limit on its free tier, gets a trimmed recent-only list, and only when needed. The AI reliably extracts the company and role from the email itself, but citing the correct row *number* out of a 900+ row list is a harder task for it and it can occasionally get that part wrong even while confident — so its row citation is cross-checked against its own company/role extraction, and discarded if they disagree. Either way (no citation, or a discarded one), a local backstop then checks your *full* job history for a row whose company **and role** both match closely (e.g. inferring the company from a sender domain like `@bbraun.com`, or catching a duplicate paste of an email you already added) — you'll see a "🔎 found by direct text match" note asking you to confirm it instead of a full AI match. Matching requires both company and role together specifically so a second, different application to a company you've already applied to before (a different role, or a past rejected attempt) doesn't get misfiled against that unrelated row
